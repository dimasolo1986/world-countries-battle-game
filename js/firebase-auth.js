...
function createErrorInternal(e, ...t) {
  if ("string" == typeof e) return E.create(e, ...t);
  {
    const r = t[0],
      n = [...t.slice(1)];
...
async function _performFetchWithErrorHandling(e, t, r) {
  ...
  try {
    const t = new NetworkTimeout(e),
      a = await Promise.race([r(), t.promise]);
    t.clearNetworkTimeout();
    var i = await a.json();
    if ("needConfirmation" in i)
      throw _makeTaggedError(e, "account-exists-with-different-credential", i);
    if (a.ok && !("errorMessage" in i)) return i;
    {
      const t = a.ok ? i.errorMessage : i.error.message,
        [r, o] = t.split(" : ");
      if ("FEDERATED_USER_ID_ALREADY_LINKED" === r)
        throw _makeTaggedError(e, "credential-already-in-use", i);
      if ("EMAIL_EXISTS" === r)
        throw _makeTaggedError(e, "email-already-in-use", i);
      if ("USER_DISABLED" === r) throw _makeTaggedError(e, "user-disabled", i);
      var s = n[r] || r.toLowerCase().replace(/[_\s]+/g, "-");
      if (o) throw _errorWithCustomMessage(e, s, o);
      _fail(e, s);
    }
  } catch (t) {
...
async function _getFinalTarget(e, t, r, n) {
  ...
  return w.includes(r) &&
    ...
    ? s._getPersistence()._getFinalTarget(a).toString()
    : a;
}
...
class UserImpl {
  ...
  static _fromJSON(e, t) {
    ...
    return (
      p && Array.isArray(p) && (f.providerData = p.map((e) => ({ ...e }))),
      o && (f._redirectEventId = o),
      f
    );
  }
...
class PersistenceUserManager {
  ...
  async getCurrentUser() {
    var e,
      t = await this.persistence._get(this.fullUserKey);
    return t
      ? "string" == typeof t
        ? (e = await getAccountInfo(this.auth, { idToken: t }).catch(() => {}))
          ? UserImpl._fromGetAccountInfoResponse(this.auth, e, t)
          : null
        : UserImpl._fromJSON(this.auth, t)
      : null;
  }
  removeCurrentUser() {
...
class AuthImpl {
  ...
  async _onStorageEvent() {
    ...
    if (!this._deleted)
      return (
        (e = await this.assertedPersistence.getCurrentUser()),
        this.currentUser || e
  ...
  async initializeCurrentUserFromIdToken(e) {
    try {
      var t = await getAccountInfo(this, { idToken: e }),
  ...
  async _getAppCheckToken() {
    ...
    var t,
      r,
      n = await this.appCheckServiceProvider
        .getImmediate({ optional: !0 })
        ?.getToken();
    return (
      n?.error &&
        ((t = "Error while retrieving App Check token: " + n.error),
        (r = []),
        A.logLevel <= u.WARN && A.warn(`Auth (${i}): ` + t, ...r)),
      n?.token
    );
  }
}
...
async function injectRecaptchaFields(e, t, r, n = !1, i = !1) {
  ...
  if ("phoneEnrollmentInfo" in i) {
    const e = i.phoneEnrollmentInfo.phoneNumber,
      t = i.phoneEnrollmentInfo.recaptchaToken;
    Object.assign(i, {
      phoneEnrollmentInfo: {
        phoneNumber: e,
        recaptchaToken: t,
    ...
    const e = i.phoneSignInInfo.recaptchaToken;
    Object.assign(i, {
      phoneSignInInfo: {
        recaptchaToken: e,
        captchaResponse: a,
    ...
    var t,
      ...
      n =
        ((n = querystringDecode(extractQuerystring(e)).deep_link_id)
          ? querystringDecode(extractQuerystring(n)).link
          : null) ||
        n ||
        r ||
        t ||
...
class GoogleAuthProvider extends BaseOAuthProvider {
  ...
  static credentialFromTaggedObject({ _tokenResponse: e }) {
    if (!e) return null;
    var { oauthIdToken: e, oauthAccessToken: t } = e;
    if (!e && !t) return null;
    try {
      return GoogleAuthProvider.credential(e, t);
    } catch {
      return null;
    }
  }
...
class SAMLAuthCredential extends AuthCredential {
  ...
  static fromJSON(e) {
    ...
    return e && t && r && e === t ? new SAMLAuthCredential(e, r) : null;
  }
  static _create(e, t) {
...
class TwitterAuthProvider extends BaseOAuthProvider {
  ...
  static credentialFromTaggedObject({ _tokenResponse: e }) {
    if (!e) return null;
    var { oauthAccessToken: e, oauthTokenSecret: t } = e;
    if (!e || !t) return null;
    try {
      return TwitterAuthProvider.credential(e, t);
    } catch {
      return null;
    }
  }
...
async function _assertLinkedStatus(e, t, r) {
  await _reloadWithoutSaving(t);
  var n = !1 === e ? "provider-already-linked" : "no-such-provider";
...
function _setActionCodeSettingsOnRequest(e, t, r) {
  _assert(0 < r.url?.length, e, "invalid-continue-uri"),
    ...
    r.iOS &&
      (_assert(0 < r.iOS.bundleId.length, e, "missing-ios-bundle-id"),
...
async function checkActionCode(e, t) {
  ...
  return (
    n.mfaInfo &&
      (i = MultiFactorInfoImpl._fromServerResponse(_castAuth(r), n.mfaInfo)),
    {
      data: {
        email:
...
class Receiver {
  ...
  async handleEvent(e) {
    ...
    s?.size &&
      ...
      (e = Array.from(s).map(async (e) => e(t.origin, i))),
      (e = await Promise.all(
        e.map(async (e) => {
          try {
...
async function _verifyPhoneNumber(e, t, r) {
  ...
  try {
    var n = "string" == typeof t ? { phoneNumber: t } : t;
    if ("session" in n) {
      const t = n.session;
      if ("phoneNumber" in n) {
...
async function injectRecaptchaV2Token(e, t, r) {
  _assert(r.type === j, e, "argument-error");
  var n = await r.verify(),
    i = (_assert("string" == typeof n, e, "argument-error"), { ...t });
  if ("phoneEnrollmentInfo" in i) {
    const e = i.phoneEnrollmentInfo.phoneNumber,
...
class PhoneAuthProvider {
  ...
  static credentialFromTaggedObject({ _tokenResponse: e }) {
    ...
    return e && t ? PhoneAuthCredential._fromTokenResponse(e, t) : null;
  }
}
...
class AuthEventManager {
  ...
  sendToConsumer(e, t) {
    var r;
    e.error && !isNullRedirectEvent(e)
      ? ((r = e.error.code?.split("auth/")[1] || "internal-error"),
...
function _open(e, t, r, n = 500, i = 600) {
  ...
  const u = window.open(t || "", o, n);
  _assert(u, e, "popup-blocked");
  try {
    u.focus();
  } catch (e) {}
  return new AuthPopup(u);
}
  ...
  se = class {
    ...
    async _openRedirect(e, t, r, n) {
      return (
        ...
        (e = await _getRedirectUrl(e, t, r, _getCurrentUrl(), n)),
...
class TotpSecret {
  ...
  generateQrCodeUrl(e, t) {
    ...
    return (
      ...
      `otpauth://totp/${t}:${e}?secret=${this.secretKey}&issuer=${t}&algorithm=${this.hashingAlgorithm}&digits=` +
        this.codeLength
    );
  }
}
