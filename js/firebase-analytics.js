...
function clearTimedOutRequest(e) {
  return (function hasInstallationRequestTimedOut(e) {
    return 1 === e.registrationStatus && e.registrationTime + v < Date.now();
  })(e)
...
function updateAuthTokenRequest(e) {
  return update(e, (e) => {
    ...
    return (function hasAuthTokenRequestTimedOut(e) {
      return 1 === e.requestStatus && e.requestTime + v < Date.now();
    })(e.authToken)
...
const P = "installations",
  ...
  internalFactory = (e) => {
    const t = e.getProvider("app").getImmediate(),
      n = _getProvider(t, P).getImmediate();
    return {
      getId: () =>
...
!(function registerInstallations() {
  t(new Component(P, publicFactory, "PUBLIC")),
    t(new Component("installations-internal", internalFactory, "PRIVATE"));
})(),
  e(I, b),
  e(I, b, "esm2020");
const M = "analytics",
  L = "https://www.googletagmanager.com/gtag/js",
  B = new (class Logger {
    constructor(e) {
      (this.name = e),
        (this._logLevel = i),
  ...
  $ = new ErrorFactory("analytics", "Analytics", {
    "already-exists":
...
function createGtagTrustedTypesScriptURL(e) {
  if (!e.startsWith(L)) {
    const t = $.create("invalid-gtag-resource", { gtagURL: e });
    return B.warn(t.message), "";
  }
  return e;
}
...
async function fetchDynamicConfigWithRetry(e, t = O, n) {
  const { appId: r, apiKey: a, measurementId: i } = e.options;
  if (!r) throw $.create("no-app-id");
  if (!a) {
    if (i) return { measurementId: i, appId: r };
    throw $.create("no-api-key");
  }
...
async function attemptFetchDynamicConfigWithRetry(
  ...
  const { appId: i, measurementId: o } = e;
  try {
    await (function setAbortableTimeout(e, t) {
      return new Promise((n, r) => {
        ...
        e.addEventListener(() => {
          clearTimeout(i),
            r($.create("fetch-throttle", { throttleEndTimeMillis: t }));
        });
      });
    })(r, t);
  } catch (e) {
  ...
  try {
    const t = await (async function fetchDynamicConfig(e) {
      const { appId: t, apiKey: n } = e,
        r = { method: "GET", headers: getHeaders(n) },
        a =
      ...
      if (200 !== i.status && 304 !== i.status) {
        ...
        throw $.create("config-fetch-failed", {
          httpStatus: i.status,
          responseMessage: e,
        });
      }
    ...
    return a.deleteThrottleMetadata(i), t;
  } catch (t) {
    const s = t;
    if (
      ...
      if ((a.deleteThrottleMetadata(i), o))
        return (
          B.warn(
    ...
    const c =
        503 === Number(s?.customData?.httpStatus)
    ...
    return (
      a.setThrottleMetadata(i, l),
      B.debug(`Calling attemptFetch again in ${c} millis`),
      attemptFetchDynamicConfigWithRetry(e, l, r, a)
    );
  }
}
...
async function _initializeAnalytics(e, t, n, r, a, i, o) {
  const s = fetchDynamicConfigWithRetry(e);
  s
    .then((t) => {
    ...
    .catch((e) => B.error(e)),
    t.push(s);
  const c = (async function validateIndexedDB() {
      if (!isIndexedDBAvailable())
        return (
          B.warn(
            $.create("indexeddb-unavailable", {
      ...
      try {
        ...
        return (
          B.warn(
            $.create("indexeddb-unavailable", { errorInfo: e?.toString() })
              .message
          ),
          !1
        );
  ...
  (function findGtagScriptOnPage(e) {
    ...
    for (const n of Object.values(t))
      if (n.src && n.src.includes(L) && n.src.includes(e)) return n;
    return null;
  })(i) || insertScriptTag(i, l.measurementId),
    _ && (a("consent", "default", _), _setConsentDefaultForInit(void 0)),
    a("js", new Date());
  const d = o?.config ?? {};
  return (
    (d.origin = "firebase"),
    (d.update = !0),
    null != u && (d.firebase_id = u),
    a("config", l.measurementId, d),
    x && (a("set", x), _setDefaultEventParametersForInit(void 0)),
    l.measurementId
  );
}
class AnalyticsService {
  ...
  _delete() {
    return delete j[this.app.options.appId], Promise.resolve();
  }
}
let j = {},
  q = [];
...
let H,
  ...
  W = !1;
function settings(e) {
  if (W) throw $.create("already-initialized");
...
function factory(e, t, n) {
  !(function warnOnBrowserContextMismatch() {
    ...
    if (
      ...
      const t = e.map((e, t) => `(${t + 1}) ${e}`).join(" "),
        n = $.create("invalid-analytics-context", { errorInfo: t });
      B.warn(n.message);
    }
  })();
  ...
  if (!r) throw $.create("no-app-id");
  if (!e.options.apiKey) {
    if (!e.options.measurementId) throw $.create("no-api-key");
    B.warn(
  ...
  if (null != j[r]) throw $.create("already-exists", { id: r });
  if (!W) {
...
function setCurrentScreen(e, t, n) {
  (e = getModularInstance(e)),
    ...
    })(U, j[e.app.options.appId], t, n).catch((e) => B.error(e));
}
  ...
  return (
    ...
    (async function internalGetGoogleAnalyticsClientId(e, t) {
      ...
      return new Promise((t, r) => {
        e("get", n, "client_id", (e) => {
          e || r($.create("no-client-id")), t(e);
        });
      });
    })(U, j[e.app.options.appId])
  );
}
...
function setAnalyticsCollectionEnabled(e, t) {
  (e = getModularInstance(e)),
    ...
    })(j[e.app.options.appId], t).catch((e) => B.error(e));
}
  ...
  (e = getModularInstance(e)),
    (async function logEvent$1(e, t, n, r, a) {
      if (a && a.global) e("event", n, r);
        ...
        const a = await t;
        e("event", n, { ...r, send_to: a });
      }
    })(U, j[e.app.options.appId], t, n, r).catch((e) => B.error(e));
}
    ...
    t(
      new Component(
        ...
        function internalFactory(e) {
          try {
            const t = e.getProvider(M).getImmediate();
            return {
              logEvent: (e, n, r) => logEvent(t, e, n, r),
              setUserProperties: (e, n) => setUserProperties(t, e, n),
            };
          } catch (e) {
            throw $.create("interop-component-reg-failed", { reason: e });
          }
        },
        "PRIVATE"
      )
